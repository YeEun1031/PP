...
namespace FileSenderApp
{
    public partial class MainForm : MetroForm
    {
        ...
        bool Master = false;        // Master 라디오버튼이 눌러졌는지 확인하는 변수
        bool Slave = false;         // Slave 라디오버튼이 눌러졌는지 확인하는 변수
        ...
        private void TcpSocketOpen()
        {
            try
            {
                ...
                IPEndPoint endPoint = new IPEndPoint(IPAddress.Parse(ServerIPName), Ethernet_PortNum);
            ...
            try
            {
                while(listenCheck)
                {
                    ...
                    if(receiveSize > 0)
                    {
                        ...
                        dirName = "\\Users\\" + Environment.UserName + "\\DeskTop\\" + fileName;
